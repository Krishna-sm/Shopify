...
import { deleteItem, incrementIntoCart, minusIntoCart, placeOrder } from '../../redux/slices/Product'
...
const Cart = () => {
  ...
  const placeOrderoFuser = ({name,email})=>{
    dispatch(placeOrder({name,email}));
    toast.success(`${name}! Thanks For shopping...`)
  }
  ...
  <div className="flex flex-col items-center border-b bg-white py-4 sm:flex-row sm:px-10 lg:px-20 xl:px-32">
    <Link to="/" className="text-2xl font-bold text-gray-800">Your Order</Link>
    <div className="mt-4 py-2 text-xs sm:mt-0 sm:ml-auto sm:text-base">
      <div className="relative">
        <ul className="relative flex w-full items-center justify-between space-x-2 sm:space-x-4">
          <li className="flex items-center space-x-3 text-left sm:space-x-4">
            <Link className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-200 text-xs font-semibold text-emerald-700" to="/"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  <div className="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
    <div className="px-4 pt-8">
     ...
     {data && data.length>0 &&  data.map((c,i)=>{
        ...
        <img className="m-2 h-24 w-28 rounded-md border object-cover object-center" src={c.image} alt />
        <div className="flex w-full flex-col px-4 py-4">
          <span className="font-semibold">{c.title}</span>
          <div className="flex justify-start items-center gap-5">
                        <button onClick={()=>IncrementIntoCart(c.id)}  className='w-10 h-10 rounded-full text-4xl bg-black text-white'>+</button>
                        <h1 className='text-3xl'>{c?.qty}</h1>
                        <button onClick={()=>MinusIntoCart(c.id)} className='w-10 h-10 rounded-full text-4xl bg-black text-white'>-</button>
                    </div>
          <p className="text-lg font-bold">${Math.floor(c.price*c.qty)}</p>
        </div>
      </div>
     }) }
      ...
      <form className="mt-5 grid gap-6">
        <div className="relative">
          ...
          <label className="peer-checked:border-2 peer-checked:border-gray-700 peer-checked:bg-gray-50 flex cursor-pointer select-none rounded-lg border border-gray-300 p-4" htmlFor="radio_1">
            <img className="w-14 object-contain" src="https://componentland.com/images/naorrAeygcJzX0SyNI4Y0.png" alt />
            <div className="ml-5">
        ...
        <div className="relative">
          ...
          <label className="peer-checked:border-2 peer-checked:border-gray-700 peer-checked:bg-gray-50 flex cursor-pointer select-none rounded-lg border border-gray-300 p-4" htmlFor="radio_2">
            <img className="w-14 object-contain" src="https://componentland.com/images/oG8xsl3xsOkwkMsrLGKM4.png" alt />
            <div className="ml-5">
              <span className="mt-2 font-semibold">Fedex Delivery</span>
              <p className="text-slate-500 text-sm leading-6">Delivery: 2-4 Days</p>
            </div>
      ...
      <div className>
        ...
        <div className="mt-6 border-t border-b py-2">
          <div className="flex items-center justify-between">
            ...
            <p className="font-semibold text-gray-900">${Math.floor(price)}.00</p>
          </div>
          <div className="flex items-center justify-between">
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <p className="text-2xl font-semibold text-gray-900">${Math.floor(price)}.00</p>
        </div>
      </div>
      <button disabled={data.length<1}  onClick={()=>placeOrderoFuser({name:"Krishna",email:"krishan@codewithKrishna.com"})} className="mt-4 mb-8 w-full rounded-md disabled:bg-gray-300 cursor-pointer bg-gray-900 px-6 py-3 font-medium text-white">Place Order</button>
    </div>
  </div>
</div>
